(function(root){
  ...
  //returns the next in function chain for the given object
  ...
  //Clears the remaining fucntion chain for the object
  _sugarless.clear = function(){
    var obj = arguments[0]; 
    var obj_id = _getId(obj);
    
    //remove the remaining functions
    var functions = _stack[obj_id]['functions'];
    functions.splice(0, functions.length);

    //reset the async flag
    _stack[obj_id]['async'] = false;
  };
